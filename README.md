...
# 초기 설계 당시와 달라진 점
mpu센서는 twi 통신을 해야하기 때문에 프로젝트 마감 시간관계상 제약이 예상되기 때문에 반려동물이 직접 접근해서 사료를 먹을 때 인식할 수 있는 적외선 센서로 변경한다. Vcc(5V), GND, 그리고 'Analog Out' 핀 1개만 ATmega128A의 ADC 포트에 연결하면 된다. CodeVisionAVR의 코드 마법사에서 ADC를 활성화하고, read_adc(0) 함수 한 줄로 센서 값을 읽을 수 있기에 mpu보다 더 쉽게 설계가 가능하다.
